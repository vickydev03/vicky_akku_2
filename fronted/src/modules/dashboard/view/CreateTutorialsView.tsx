...
const videoEntrySchema = z.object({
  video: z.instanceof(File, { message: "Select a video file" }),
  title: z
    .string()
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  accept="video/*"
                  disabled={isBusy}
                  className="h-10 rounded-xl border-neutral-200 bg-white text-sm shadow-sm focus:ring-2 focus:ring-neutral-900/10 transition-all"
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    field.onChange(file ?? undefined);
                    if (file) onFileSelected(file, index);
                  }}
                />
              </FormControl>
              ...
              {/* ── Upload progress shown here, under the file input ── */}
              {uploadItem && (
                <InlineUploadStatus
                  item={uploadItem}
                  onPause={onPause}
